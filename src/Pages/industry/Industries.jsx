...
import ViewEditIndustry from '../../components/Popups/ViewEditIndustry';
...
const Industries = () => {
    ...
    const deleteInspire = async () => {
        ...
        try {
            const res = await callApi("/industries/removeIndustry", "post", value)
        ...
        if (!inspirePopup) {
            (async () => {
                try {
                    ...
                    const response = await callApi("/industries/getIndustriesWithFullDetails", "post", payload)
                    if (response?.status === "Success"
                    ) {
                        setallInspires(response?.data?.industries)
                    }
                } catch (error) {
                    console.log(error);
                }
    ...
    return (
        <div className='bscontainer-fluid'>
            <ViewEditIndustry id="job-modal" data={inspireRow} mode={inspireMode} modalOpen={inspirePopup} onClose={() => setinspirePopup(false)} />
                ...
                <div className='col-12  mb-5'>
                    <div className='mb-3'>
                        <ul className="inline-flex flex-wrap text-sm font-medium">
                            ...
                            <li className="flex items-center">
                                <Link to="/industry" className="text-slate-500 hover:text-indigo-500" href="#0">Industry</Link>
                            </li>
                        </ul>
                    </div>
                    <Link to="/industry/create-industry" className="p-2 flex w-[15%] items-center rounded-sm bg-red-500 hover:bg-green-600 text-white" >
                        <svg className="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                            <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                        </svg>
                        <span className="ml-2">Create Industry</span>
                    </Link>
                </div>
                <div className='col-12 border'>
                    <div className="bg-white shadow-lg rounded-sm border border-slate-200 relative">
                        <header className="px-5 py-4">
                            <h2 className="font-semibold text-slate-800">All Industry <span className="text-slate-400 font-medium">{allInspires?.length}</span></h2>
                        </header>
                        <div>
                                        ...
                                        {allInspires.map((faq) => {
                                            return (
                                                <tr key={faq._id}>
                                                    ...
                                                    <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                        <div className="text-left">{faq?.industryName}</div>
                                                    </td>
                                                    <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                        <div className="text-left">{faq?.industryType}</div>
                                                    </td>
                                                    <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                        <div className="text-left">{Truncate(faq.description, 20)}</div>
                                                    </td>
