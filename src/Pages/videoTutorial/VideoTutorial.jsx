...
import ViewEditVideo from '../../components/Popups/ViewEditVideo';
...
const VideoTutorial = () => {
    ...
    const deleteInspire = async () => {
        ...
        try {
            const res = await callApi("/jobcategories/removecategory", "post", value)
            // const res = await callApi("/tutorials/removeTutorial", "post", value)
            ...
            (async () => {
                try {
                    ...
                    const response = await callApi("/jobcategories/getcategories", "post", payload)
                              // const res = await callApi("/tutorials/getTutorialsWithFullDetails", "post", value)
    ...
    return (
        <div className='bscontainer-fluid'>
            <ViewEditVideo id="job-modal" data={inspireRow} mode={inspireMode} modalOpen={inspirePopup} onClose={() => setinspirePopup(false)} />
                ...
                <div className='col-12  mb-5'>
                    <div className='mb-3'>
                        <ul className="inline-flex flex-wrap text-sm font-medium">
                            ...
                            <li className="flex items-center">
                                <Link to="/tutorial" className="text-slate-500 hover:text-indigo-500" href="#0">Video Tutorial</Link>
                            </li>
                        </ul>
                    </div>

                    <Link to="create-tutorial" className="p-2 flex items-center rounded-sm w-[20%] bg-red-500 hover:bg-green-600 text-white" >
                        <svg className="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                            <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                        </svg>
                        <span className="ml-2">Create Video Tutorial</span>
                    </Link>
                </div>
                <div className='col-12 border'>
                    <div className="bg-white shadow-lg rounded-sm border border-slate-200 relative">
                        <header className="px-5 py-4">
                            <h2 className="font-semibold text-slate-800">All Tutorials <span className="text-slate-400 font-medium">{allInspires.length}</span></h2>
                        </header>
                    </div>
                    <div>
                            ...
                            <table className="table-auto w-full">
                                <thead className="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                            <div className="font-semibold text-left"> TOTURIAL NAME</div>
                                        </th>
                                        <th className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    ...
                                    {allInspires.map((inspire) => {
                                        return (
                                            <tr key={inspire._id}>
                                                ...
                                                <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                    <div className="text-left">{inspire.jobCategoryTitle}</div>
                                                </td>
