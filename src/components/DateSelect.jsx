...
function DateSelect() {
  ...
  return (
    <div className="relative">
      <button
        ref={trigger}
        className="p-2 w-[180px] flex items-center border justify-between  bg-white border-slate-200 hover:border-slate-300 text-slate-500 hover:text-slate-600"
            ...
            options.map(option => {
              return (
                <button
                  key={option.id}
                  tabIndex="0"
                  className={`flex items-center  hover:bg-slate-50 py-1 px-3 cursor-pointer ${option.id === selected && 'text-red-500'}`}
